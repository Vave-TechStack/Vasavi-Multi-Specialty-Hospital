...
export function AppointmentForm({ compact = false }: { compact?: boolean }) {
  ...
  const [doctors, setDoctors] = useState<Array<{ id: string; user?: { name?: string }; specialization?: string; department?: { name?: string } }>>([]);
    ...
    async function load() {
      try {
        const [deptRes, doctorRes] = await Promise.all([
          fetch(`${apiUrl}/public/departments`),
          fetch(`${apiUrl}/public/doctors?limit=20`),
        ]);
  ...
  return (
    <form onSubmit={submit} className={`grid gap-4 ${compact ? 'md:grid-cols-2' : ''}`}>
      ...
      <select name="department" required defaultValue="" className="rounded-xl border-slate-200 px-4 py-3 text-sm focus:border-primary focus:ring-primary" disabled={loading}><option value="" disabled>{loading ? 'Loading departments…' : 'Select department'}</option>{departments.map(d=><option key={d.id} value={d.name}>{d.name}</option>)}</select>
      <select name="preferredDoctor" defaultValue="" className="rounded-xl border-slate-200 px-4 py-3 text-sm focus:border-primary focus:ring-primary" disabled={loading}><option value="">Preferred doctor</option>{doctors.map(d=><option key={d.id} value={d.user?.name || ''}>{d.user?.name} {d.specialization ? `- ${d.specialization}` : ''}</option>)}</select>
